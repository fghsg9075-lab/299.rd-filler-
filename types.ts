...
export interface SystemSettings {
  ...
  wheelRewards?: any[];
  chatCost?: number;
  chatCooldown?: number; // NEW: Cooldown in seconds (Admin Controlled)
  dailyReward?: number;
  signupBonus?: number;
...
export interface GiftCode {
  id: string;
  code: string;
  type: 'CREDITS' | 'SUBSCRIPTION'; // New Type
  amount: number; // Used for Credits
  
  // Subscription Specifics
  subTier?: 'WEEKLY' | 'MONTHLY' | '3_MONTHLY' | 'YEARLY' | 'LIFETIME' | 'CUSTOM';
...
export interface LessonContent {
  ...
  usedApiKey?: string; // NEW: Track which API key generated this content
}
