...
import { UniversalSupport } from './UniversalSupport'; // REPLACED ChatHub
import { UniversalInfoPage } from './UniversalInfoPage';
import { FeatureComparisonModal } from './FeatureComparisonModal';
import { FeaturesShowcase } from './FeaturesShowcase'; // REPLACED FeatureComparisonModal logic
import { ExpiryPopup } from './ExpiryPopup';
import { SubscriptionHistory } from './SubscriptionHistory';
import { ProfessionalStartupPopup } from './ProfessionalStartupPopup';
import { SmartSession } from './SmartSession'; // NEW
...
export const StudentDashboard: React.FC<Props> = ({ user, dailyStudySeconds, onSubjectSelect, onRedeemSuccess, settings, onStartWeeklyTest, activeTab, onTabChange, setFullScreen, onNavigate, isImpersonating }) => {
  ...
  // REPLACED CHAT WITH SUPPORT MODAL (Now Universal Support)
  const [showSupportModal, setShowSupportModal] = useState(false);
  
  // SMART SESSION STATE
  const [showSmartSession, setShowSmartSession] = useState(false);
  ...
  // handleSupportEmail removed in favor of UniversalSupport component
  ...
  const claimDailyReward = () => {
      ...
      // STRICT REWARD LOGIC: Free=3, Basic=10, Ultra=20
      let finalReward = 3; // Default Free
  ...
  // SMART SESSION NAVIGATOR
  const handleSmartSessionNavigate = (content: any) => {
      // We need to set state to show player
      // But content object from SmartSession is flat. 
      // We need to construct dummy Subject/Chapter objects or use what we have.
      // SmartSession returns { subjectName: string, title: string (Chapter), ... }
      
      const dummySubject: Subject = { 
          id: content.subjectName, 
      ...
      const dummyChapter: Chapter = {
          id: content.id.replace(/^(pdf-|vid-|mcq-)/, ''), // Extract ID
          title: content.title, // Actually this is usually "Notes" or "Video", need Chapter Title
          description: content.subtitle
      };
      
      // WAIT! SmartSession returns LessonContent. 
      // LessonContent has `title` as "Chapter Title" usually in our fetching logic?
      // Let's check fetchLessonContent. Yes, title is chapter title.
      ...
      if (content.type.includes('VIDEO')) {
          onTabChange('VIDEO');
      } else if (content.type.includes('MCQ')) {
          onTabChange('MCQ');
      } else {
          onTabChange('PDF');
      }
              ...
              <div className="space-y-6 pb-24">
                  ...
                  <div>
                      ...
                      <div className="grid grid-cols-4 gap-3">
                          ...
                          {/* SMART SESSION BUTTON (Only 9-12) */}
                          {['9','10','11','12'].includes(user.classLevel || '') && (
                              <button onClick={() => setShowSmartSession(true)} className="aspect-square bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col items-center justify-center gap-1 hover:bg-slate-50 transition-all group">
                                  <div className="bg-indigo-50 p-2 rounded-full group-hover:bg-indigo-100 transition-colors"><Search size={20} className="text-indigo-600" /></div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-20">
        ...
        {/* APP FEATURES MODAL (New Showcase) */}
        {showFeaturesModal && (
            <FeaturesShowcase onClose={() => setShowFeaturesModal(false)} />
        )}
        ...
        {/* SMART SESSION MODAL */}
        <SmartSession 
            isOpen={showSmartSession}
            user={user}
            onClose={() => setShowSmartSession(false)}
            onNavigateToContent={handleSmartSessionNavigate}
        />
        ...
        {/* PROFESSIONAL STARTUP POPUP */}
        {showFeaturePopup && (
            <ProfessionalStartupPopup 
