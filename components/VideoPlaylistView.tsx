...
import { PlayCircle, Lock, ArrowLeft, Crown, AlertCircle, CheckCircle, Youtube, Maximize, Trash2, Edit } from 'lucide-react';
...
export const VideoPlaylistView: React.FC<Props> = ({ 
  ...
  // ADMIN ACTIONS
  const handleAdminDelete = async (index: number) => {
      ...
      // SAVE
      saveContentToStorage(newPlaylist);
  };

  const handleAdminEdit = async (index: number) => {
      const video = playlist[index];
      ...
      const newAccess = prompt("Access Level (FREE / BASIC / ULTRA):", video.access || 'ULTRA');
      if (newAccess === null) return;

      const updatedVideo = { ...video, title: newTitle || video.title, url: newUrl || video.url, access: newAccess };
      const newPlaylist = [...playlist];
      newPlaylist[index] = updatedVideo as any;
      setPlaylist(newPlaylist);
      
      saveContentToStorage(newPlaylist);
  };

  const saveContentToStorage = async (newPlaylist: any[]) => {
      let key = '';
      if (chapter.id === 'UNIVERSAL') {
          key = 'nst_universal_playlist';
      } else {
      ...
      // Update Local State (contentData) to preserve other fields
      let existingData = contentData || {};
      let updatedData = { ...existingData };

      // Update specific field based on mode
      if (syllabusMode === 'SCHOOL') {
          updatedData.schoolVideoPlaylist = newPlaylist;
          // Legacy sync
          updatedData.videoPlaylist = newPlaylist; 
      } else {
          updatedData.competitionVideoPlaylist = newPlaylist;
      }

      localStorage.setItem(key, JSON.stringify(updatedData));
      await saveChapterData(key, updatedData);
      setAlertConfig({isOpen: true, message: "Playlist Updated!"});
  };
                                   ...
                                   {user.role === 'ADMIN' ? (
                                       <div className="flex gap-2">
                                           <button 
                                               ...
                                               className="flex-1 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-lg text-xs flex items-center justify-center gap-1"
                                           >
                                               <PlayCircle size={14} /> Play
                                           </button>
                                           <button 
                                               onClick={() => handleAdminEdit(idx)}
                                               className="p-2 bg-blue-100 text-blue-600 rounded-lg hover:bg-blue-200"
                                           >
                                               <Edit size={14} />
                                           </button>
                                           <button 
                                               onClick={() => handleAdminDelete(idx)}
