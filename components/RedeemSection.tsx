...
export const RedeemSection: React.FC<Props> = ({ user, onSuccess }) => {
  ...
  const handleRedeem = async () => {
    ...
    try {
        ...
        // 3. Determine Reward Type (Credits vs Subscription)
        const type = targetCode.type || 'CREDITS'; // Default to Credits for old codes
        ...
        if (type === 'SUBSCRIPTION') {
             ...
             if (targetCode.subTier === 'WEEKLY') {
                 endDate = new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000);
             } else if (targetCode.subTier === 'MONTHLY') {
                 endDate = new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000);
             } else if (targetCode.subTier === '3_MONTHLY') {
                 endDate = new Date(now.getTime() + 90 * 24 * 60 * 60 * 1000);
             } else if (targetCode.subTier === 'YEARLY') {
                 endDate = new Date(now.getTime() + 365 * 24 * 60 * 60 * 1000);
             } else if (targetCode.subTier === 'LIFETIME') {
                 endDate = null;
             } else if (targetCode.subTier === 'CUSTOM' && targetCode.customDuration) {
                  const d = targetCode.customDuration;
                  const totalMs = (d.years || 0) * 365 * 24 * 3600 * 1000 +
                                  (d.months || 0) * 30 * 24 * 3600 * 1000 +
                                  (d.days || 0) * 24 * 3600 * 1000 +
                                  (d.hours || 0) * 3600 * 1000 +
                                  (d.minutes || 0) * 60 * 1000 +
                                  (d.seconds || 0) * 1000;
             ...
             const historyEntry = {
                id: `hist-${Date.now()}`,
                tier: targetCode.subTier,
                level: targetCode.subLevel,
                startDate: now.toISOString(),
                endDate: endDate ? endDate.toISOString() : 'LIFETIME',
                durationHours: endDate ? Math.round((endDate.getTime() - now.getTime()) / 3600000) : 999999,
                price: 0,
                originalPrice: 0,
                isFree: true,
                grantSource: 'REDEEM_CODE',
                grantedBy: targetCode.code // Track which code was used
             };

             updatedUser = {
                 ...updatedUser,
                 subscriptionTier: targetCode.subTier,
                 subscriptionLevel: targetCode.subLevel || 'BASIC',
                 subscriptionEndDate: endDate ? endDate.toISOString() : undefined,
                 isPremium: true,
                 // @ts-ignore
                 subscriptionHistory: [historyEntry, ...(user.subscriptionHistory || [])]
             };
             
             setMsg(`🎉 Subscription Activated: ${targetCode.subTier} (${targetCode.subLevel || 'BASIC'})!`);

        } else {
             // Credit Logic
             const amountToAdd = targetCode.amount || 0;
             const newCredits = (user.credits || 0) + amountToAdd;
             updatedUser.credits = newCredits;
             setMsg(`Success! Added ${amountToAdd} Premium Credits.`);
        }
