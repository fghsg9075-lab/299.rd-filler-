...
export const McqView: React.FC<Props> = ({ 
  ...
  const triggerMcqStart = (mode: 'PRACTICE' | 'TEST', data: any) => {
    // MCQ LIMIT LOGIC (Free: 50, Basic: 100, Ultra: Unlimited)
    let limit = 50;
    const isSubscribed = user.isPremium && user.subscriptionEndDate && new Date(user.subscriptionEndDate) > new Date();
  ...
  const handleViewAnalysis = (cost: number) => {
      // 0. Check Plan Level (Free users cannot view analysis even with coins)
      const isSubscribed = user.isPremium && user.subscriptionEndDate && new Date(user.subscriptionEndDate) > new Date();
      if (!isSubscribed) {
          setAlertConfig({isOpen: true, title: "Premium Feature", message: "Analysis is only available for Basic & Ultra members."});
          return;
      }
  ...
  return (
    <div className="bg-white min-h-screen pb-20 animate-in fade-in slide-in-from-right-8">
       ...
       <div className="p-6 space-y-4">
           ...
           {user.role === 'ADMIN' && (
               <button 
                   onClick={() => setAlertConfig({isOpen: true, title: "Admin Action", message: "To add/edit questions, go to Admin Dashboard > Content Manager > MCQ."})}
                   className="w-full p-4 rounded-xl border-2 border-dashed border-slate-300 text-slate-500 font-bold flex items-center justify-center gap-2 hover:bg-slate-50 hover:text-slate-700"
               >
                   ✏️ Edit Questions (Admin)
               </button>
           )}
