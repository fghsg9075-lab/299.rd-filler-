...
import { FileText, Lock, ArrowLeft, Crown, Star, CheckCircle, AlertCircle, Globe, Maximize, Layers, HelpCircle, Minus, Plus, Edit, Trash2 } from 'lucide-react';
...
export const PdfView: React.FC<Props> = ({ 
  ...
  const saveContent = async (newData: any) => {
      ...
      setAlertConfig({isOpen: true, message: "Content Updated!"});
  };

  const handleAdminEditMain = (type: 'FREE' | 'PREMIUM') => {
      let newData = { ...contentData };
      if (type === 'FREE') {
          const link = prompt("Edit Free PDF Link:", newData.schoolPdfLink || newData.freeLink);
          if (link !== null) {
              if (syllabusMode === 'SCHOOL') { newData.schoolPdfLink = link; newData.freeLink = link; }
              else newData.competitionPdfLink = link;
              saveContent(newData);
          }
      } else {
          const link = prompt("Edit Premium PDF Link:", newData.schoolPdfLink || newData.premiumLink); // Logic for premium might differ based on mode, using simplified access
          const price = prompt("Edit Price:", newData.price || 5);
          if (link !== null) {
               if (syllabusMode === 'SCHOOL') { newData.premiumLink = link; newData.schoolPdfPrice = Number(price); }
               else { newData.competitionPdfLink = link; newData.competitionPdfPrice = Number(price); }
               saveContent(newData);
          }
      }
  };
           ...
           {loading ? (
               ...
               <>
                   ...
                   <div className="relative group">
                       ...
                       {/* ADMIN EDIT - FREE */}
                       {user.role === 'ADMIN' && (
                           <button onClick={(e) => { e.stopPropagation(); handleAdminEditMain('FREE'); }} className="absolute bottom-2 right-2 p-2 bg-slate-100 rounded-lg hover:bg-slate-200 text-slate-600 z-20">
                               <Edit size={16} />
                           </button>
                       )}
                       {/* INFO BUTTON - FREE */}
                   ...
                   <div className="relative group">
                       ...
                       {/* ADMIN EDIT - PREMIUM */}
                       {user.role === 'ADMIN' && (
                           <button onClick={(e) => { e.stopPropagation(); handleAdminEditMain('PREMIUM'); }} className="absolute bottom-2 right-2 p-2 bg-slate-100 rounded-lg hover:bg-slate-200 text-slate-600 z-20">
                               <Edit size={16} />
                           </button>
                       )}
                       {/* INFO BUTTON - PREMIUM */}
