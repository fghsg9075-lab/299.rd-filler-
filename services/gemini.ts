...
export const fetchChapters = async (
  ...
  try {
    const { result: data } = await executeWithRotation(async (ai) => {
...
export const fetchLessonContent = async (
  ...
  if (type === 'MCQ_ANALYSIS' || type === 'MCQ_SIMPLE') {
      ...
      const { result: data, key: usedKey } = await executeWithRotation(async (ai) => {
          const response = await ai.models.generateContent({
              model: modelName,
              contents: prompt,
              config: { responseMimeType: "application/json" }
  ...
  const { result: text, key: usedKey } = await executeWithRotation(async (ai) => {
      const response = await ai.models.generateContent({
          model: modelName,
          contents: prompt,
      });
